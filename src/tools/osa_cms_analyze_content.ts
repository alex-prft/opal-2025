// src/tools/osa_cms_analyze_content.ts
 ...
 * Connects to CMS Analytics and Content Assessment APIs for comprehensive site evaluation
 */
async function osaCmsAnalyzeContent(params: WebsiteContentAnalysisParams): Promise<WebsiteContentAnalysisResponse> {
  const startTime = Date.now();
  const correlationId = `cms-analysis-${Date.now()}-${Math.random().toString(36).substring(7)}`;
  const analysisDepth = params.analysis_depth || 'comprehensive';
  const websiteUrl = params.website_url || 'https://freshproduce.com';

  console.log('🌐 [CMS Content Analysis] Starting comprehensive website analysis', {
    correlationId,
  ...
  try {
    ...
    if (params.include_performance_assessment) {
      try {
        performanceAssessment = await osaCmsAssessContentPerformance(params, correlationId);
        console.log('✅ [CMS Content Analysis] Performance assessment completed', { correlationId });
      } catch (error) {
        console.error('❌ [CMS Content Analysis] Performance assessment failed:', error);
    ...
    if (params.include_seo_analysis) {
      try {
        ...
        console.log('✅ [CMS Content Analysis] SEO analysis completed', { correlationId });
      } catch (error) {
        console.error('❌ [CMS Content Analysis] SEO analysis failed:', error);
    ...
    if (params.include_accessibility_check) {
      try {
        ...
        console.log('✅ [CMS Content Analysis] Accessibility audit completed', { correlationId });
      } catch (error) {
        console.error('❌ [CMS Content Analysis] Accessibility audit failed:', error);
    ...
    if (params.include_content_audit) {
      try {
        ...
        console.log('✅ [CMS Content Analysis] Content audit completed', { correlationId });
      } catch (error) {
        console.error('❌ [CMS Content Analysis] Content audit failed:', error);
    ...
    console.log('✅ [CMS Content Analysis] Analysis completed', {
      correlationId,
    ...
    console.error('❌ [CMS Content Analysis] Analysis failed:', {
      correlationId,
...
async function analyzeWebsiteContentCore(params: WebsiteContentAnalysisParams, correlationId: string) {
  console.log('🔍 [Core CMS Analysis] Analyzing website content structure');

  // Connect to real CMS Analysis API
  const websiteEndpoint = process.env.CMS_ANALYSIS_API || '/api/cms/content-analysis';
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`CMS Analysis API returned ${response.status}: ${response.statusText}`);
    }

    const websiteData = await response.json();
    console.log('✅ [Core CMS Analysis] Real website data retrieved', { correlationId });
    ...
    console.error(`❌ [Core CMS Analysis] Failed to connect to real CMS data:`, error);
    throw new Error(`Unable to analyze CMS content: ${error instanceof Error ? error.message : 'CMS API unavailable'}`);
  }
}
 ...
 */
async function osaCmsAssessContentPerformance(params: WebsiteContentAnalysisParams, correlationId: string): Promise<PerformanceMetrics> {
  console.log('📊 [CMS Performance Assessment] Analyzing content performance metrics');

  try {
    // Connect to real CMS Performance API
    const performanceEndpoint = process.env.CMS_PERFORMANCE_API || '/api/cms/performance-assessment';
    ...
    if (!response.ok) {
      throw new Error(`CMS Performance API returned ${response.status}: ${response.statusText}`);
    }

    const performanceData = await response.json();
    console.log('✅ [CMS Performance Assessment] Real performance data retrieved', { correlationId });

    return performanceData.performance_metrics;

  } catch (error) {
    console.error('❌ [CMS Performance Assessment] Failed to retrieve real performance data:', error);
...
async function performSEOAnalysis(params: WebsiteContentAnalysisParams, correlationId: string): Promise<SEOAnalysis> {
  ...
  try {
    ...
    const seoEndpoint = process.env.SEO_ANALYSIS_API || '/api/cms/seo-analysis';
...
async function performAccessibilityAudit(params: WebsiteContentAnalysisParams, correlationId: string): Promise<AccessibilityAudit> {
  ...
  try {
    ...
    const accessibilityEndpoint = process.env.ACCESSIBILITY_API || '/api/cms/accessibility-audit';
...
async function performContentAudit(params: WebsiteContentAnalysisParams, correlationId: string): Promise<ContentAudit> {
  ...
  try {
    ...
    const auditEndpoint = process.env.CONTENT_AUDIT_API || '/api/cms/content-audit';
...
function createFreshProduceFallbackWebsiteAnalysis(
  ...
  console.log('🔄 [Fallback CMS Analysis] Providing industry-specific fallback data');
...
tool({
  ...
  description: "Analyze website content comprehensively with CMS integration including optional performance assessment, SEO analysis, accessibility audit, and content audit. Provides detailed content quality analysis, performance metrics, SEO recommendations, accessibility compliance checking, and content optimization opportunities. Includes CMS-specific content performance functionality for comprehensive content evaluation.",
  parameters: [
    {
      name: "website_url",
