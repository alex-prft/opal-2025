// src/tools/osa_store_workflow_data.ts
...
async function storeWorkflowData(params: WorkflowDataParams): Promise<WorkflowDataResponse> {
  ...
  console.log('💾 [Workflow Data Storage] Storing workflow data and state', {
    correlationId,
  ...
  try {
    ...
    if (params.update_agent_progress) {
      try {
        ...
        console.log('✅ [Workflow Data Storage] Agent progress updated', { correlationId });
      } catch (error) {
        console.error('❌ [Workflow Data Storage] Agent progress update failed:', error);
    ...
    if (params.store_intermediate_results) {
      try {
        ...
        console.log('✅ [Workflow Data Storage] Intermediate results stored', { correlationId });
      } catch (error) {
        console.error('❌ [Workflow Data Storage] Intermediate results storage failed:', error);
    ...
    if (params.trigger_workflow_events) {
      try {
        ...
        console.log('✅ [Workflow Data Storage] Workflow events triggered', { correlationId });
      } catch (error) {
        console.error('❌ [Workflow Data Storage] Workflow events triggering failed:', error);
    ...
    console.log('✅ [Workflow Data Storage] Data storage completed', {
      correlationId,
    ...
    return {
      ...
      _metadata: {
        storage_system: 'workflow_data_management',
        processing_time_ms: processingTime,
    ...
    console.error('❌ [Workflow Data Storage] Data storage failed:', {
      correlationId,
      workflow_id: params.workflow_id,
...
function createFreshProduceFallbackStorage(
  ...
  return {
    ...
    timestamp: new Date().toISOString(),
    _metadata: {
      storage_system: 'fallback_storage',
