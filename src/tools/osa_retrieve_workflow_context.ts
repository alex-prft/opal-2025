// src/tools/osa_retrieve_workflow_context.ts
...
async function retrieveWorkflowContext(params: WorkflowContextParams): Promise<WorkflowContextResponse> {
  ...
  const correlationId = params.correlation_id || `context-${Date.now()}-${Math.random().toString(36).substring(7)}`;
  const contextDepth = params.context_depth || 'standard';

  console.log('🔍 [Workflow Context] Retrieving workflow context', {
    correlationId,
  ...
  try {
    ...
    if (params.include_agent_history) {
      try {
        ...
        console.log('✅ [Workflow Context] Agent history retrieved', { correlationId });
      } catch (error) {
        console.error('❌ [Workflow Context] Agent history retrieval failed:', error);
    ...
    if (params.include_data_lineage) {
      try {
        ...
        console.log('✅ [Workflow Context] Data lineage retrieved', { correlationId });
      } catch (error) {
        console.error('❌ [Workflow Context] Data lineage retrieval failed:', error);
    ...
    if (params.include_user_context) {
      try {
        ...
        console.log('✅ [Workflow Context] User context retrieved', { correlationId });
      } catch (error) {
        console.error('❌ [Workflow Context] User context retrieval failed:', error);
    ...
    console.log('✅ [Workflow Context] Context retrieval completed', {
      correlationId,
    ...
    return {
      ...
      _metadata: {
        data_source: 'workflow_management_api',
        processing_time_ms: processingTime,
        context_depth: contextDepth,
        agent_history_included: !!agentHistory,
        data_lineage_included: !!dataLineage,
    ...
    console.error('❌ [Workflow Context] Context retrieval failed:', {
      correlationId,
      workflow_id: params.workflow_id,
...
async function fetchCoreWorkflowContext(params: WorkflowContextParams, correlationId: string): Promise<WorkflowContext> {
  console.log('🔄 [Core Context] Fetching core workflow context');
  ...
  try {
    ...
    console.log('✅ [Core Context] Real context data retrieved', { correlationId });

    return contextData.workflow_context;

  } catch (error) {
    console.error('❌ [Core Context] Failed to connect to real workflow context:', error);
...
async function fetchAgentExecutionHistory(params: WorkflowContextParams, correlationId: string): Promise<AgentExecutionHistory[]> {
  console.log('📚 [Agent History] Fetching agent execution history');
  ...
  try {
    ...
    console.log('✅ [Agent History] Real history data retrieved', { correlationId });

    return historyData.agent_history;

  } catch (error) {
    console.error('❌ [Agent History] Failed to retrieve real agent history:', error);
...
async function fetchDataLineage(params: WorkflowContextParams, correlationId: string): Promise<DataLineage> {
  console.log('🔗 [Data Lineage] Fetching data lineage information');
  ...
  try {
    ...
    console.log('✅ [Data Lineage] Real lineage data retrieved', { correlationId });

    return lineageData.data_lineage;

  } catch (error) {
    console.error('❌ [Data Lineage] Failed to retrieve real data lineage:', error);
...
async function fetchUserContext(params: WorkflowContextParams, correlationId: string): Promise<UserContext> {
  console.log('👤 [User Context] Fetching user context information');
  ...
  try {
    ...
    const contextData = await response.json();
    console.log('✅ [User Context] Real user context retrieved', { correlationId });

    return contextData.user_context;

  } catch (error) {
    console.error('❌ [User Context] Failed to retrieve real user context:', error);
...
function createFreshProduceFallbackContext(
  ...
  console.log('🔄 [Fallback Context] Providing fresh produce industry-specific fallback context');
